-- ============================================================
-- 水库闸门调度系统 - PostgreSQL 初始化脚本
-- 数据库名: reservoir_scheduling
-- 说明: 此脚本创建数据库、所有业务表结构及索引
-- 用法: 使用超级用户执行  psql -U postgres -f init.sql
-- ============================================================

-- 1. 创建数据库
SELECT 'CREATE DATABASE reservoir_scheduling'
WHERE NOT EXISTS (SELECT FROM pg_database WHERE datname = 'reservoir_scheduling')\gexec

-- 2. 连接数据库
\c reservoir_scheduling

-- 3. 启用扩展（pgcrypto 用于 gen_random_uuid）
...
CREATE INDEX IF NOT EXISTS idx_water_supply_impacts_deleted_at ON water_supply_impacts(deleted_at);
